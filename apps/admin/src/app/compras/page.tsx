...
export default async function ComprasPage({
  ...
  const [{ data: purchases }, { data: purchaseProducts }, { data: suppliers }, { data: expenses }, { data: categories }] =
    await Promise.all([
      supabase
        .from('purchases')
        .select(`
      ...
      supabase
        ...
        .select(
          'id, stock, price, avg_unit_cost, last_unit_cost, min_stock, product:products ( id, name, unit, sku, weigh_at_fulfillment )',
        )
        .eq('branch_id', tenant.branchId)
  ...
  return (
    <AdminShell title="Compras" subtitle={tenant.branchName}>
      <PurchasesManager
        ...
        initialCategories={(categories ?? []) as Array<{
          id: string;
          name: string;
          sort_order: number;
        }>}
