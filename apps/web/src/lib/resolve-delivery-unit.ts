...
export async function resolveDeliveryUnitId(
  ...
  if (!label) {
    throw new Error('Ingresa tu departamento para la entrega.');
  }
  ...
  if (!buildingId) {
    ...
    if (buildingError || !createdBuilding) {
      throw new Error(buildingError?.message ?? 'No se pudo registrar el departamento');
    }
  ...
  if (unitError || !createdUnit) {
    throw new Error(unitError?.message ?? 'No se pudo registrar el departamento');
  }
