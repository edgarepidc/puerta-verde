-- Capital contributions and rare operating income (refunds, asset sales).
-- Contributions do not enter estimated net profit; operating income does.

create type public.income_entry_type as enum ('contribution', 'operating');
...
create table public.income_entries (
  ...
  entry_date date not null default (timezone('America/Mexico_City', now()))::date,
  notes text,
  created_by uuid references auth.users (id) on delete set null,
  created_at timestamptz not null default now()
);

create index income_entries_branch_idx on public.income_entries (branch_id, entry_date desc);

alter table public.income_entries enable row level security;

create policy "staff manage income entries" on public.income_entries
...
begin
  ...
  select
    ...
    round(v_revenue - v_cogs - v_fixed - v_variable - v_visit + v_other_income, 2),
    v_orders;
end;
$$;
