-- Fix ambiguous order_number in place_guest_order RETURNING clause.
-- RETURNS TABLE(... order_number ...) creates an OUT param that conflicts
-- with orders.order_number when unqualified in RETURNING.
...
grant execute on function public.place_guest_order(text, text, text, public.fulfillment_type, uuid, text, jsonb) to anon, authenticated, service_role;
