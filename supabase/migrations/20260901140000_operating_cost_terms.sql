-- Operating costs apply from the period they were turned on, without rewriting the past.
-- Pausing closes the open term the day before the viewed period; history stays.

create table public.branch_operating_cost_terms (
  id uuid primary key default gen_random_uuid(),
  cost_id uuid not null references public.branch_operating_costs (id) on delete cascade,
  start_date date not null,
  end_date date,
  created_at timestamptz not null default now(),
  constraint branch_operating_cost_terms_range_ok check (end_date is null or end_date >= start_date)
);

create index branch_operating_cost_terms_cost_idx
  on public.branch_operating_cost_terms (cost_id, start_date);

alter table public.branch_operating_cost_terms enable row level security;

create policy "staff read operating cost terms" on public.branch_operating_cost_terms
  ...
  using (
    exists (
      ...
      where c.id = cost_id
        and public.is_staff_of_branch(c.branch_id)
    )
  );

create policy "staff manage operating cost terms" on public.branch_operating_cost_terms
...
insert into public.branch_operating_cost_terms (cost_id, start_date, end_date)
select
  c.id,
  (date_trunc('month', timezone('America/Mexico_City', c.created_at)))::date,
  case
    when c.is_active then null
    else ((date_trunc('month', timezone('America/Mexico_City', now())))::date - 1)
  end
from public.branch_operating_costs c
where c.is_active
   or (date_trunc('month', timezone('America/Mexico_City', c.created_at)))::date
      <= ((date_trunc('month', timezone('America/Mexico_City', now())))::date - 1);
